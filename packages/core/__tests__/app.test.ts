...
test("app boot fail ", function (done) {
    ...
    app.bootstrap([new TestBootLoader(), new TestUnOkBootLoader()]).then(onBootEnd, onBootEnd);
});
test("app boot success", function (done) {
    ...
    app.bootstrap([new TestBootLoader()]).then(onBootEnd, onBootEnd);
});
test("app delay boot success", function (done) {
    ...
    app.bootstrap([new TestDelayOkBootLoader()]).then(onBootEnd, onBootEnd);
});
test("testModule instance is exit", function (done) {
    const app = new App();
    app.bootstrap([new TestBootLoader()]).then(() => {
        app.init();
        expect(app.getModule("test1") !== null && app.getModule("test2") !== null).toBe(true);
        done();
    });
});
